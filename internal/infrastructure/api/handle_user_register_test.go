...
func TestHandleUserRegister(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		registerService := mock_api.NewMockuserRegisterService(ctrl)
		registerService.EXPECT().Register(context.Background(), login, gomock.Any()).Return(userID, nil)
		...
		c.SetPath(path)
		handler := HandleUserRegister(registerService)
