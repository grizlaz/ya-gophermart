...
// лучше для каждого репозитория свой тип?
// нормально ли, что в каждом поднимаются миграции? В какой-то одной поднимать их? Или разбить миграции так же по доменам (на будущее, не в этом проекте :) )
// сейчас есть дублирование методов в репозиториях, как лучше это фиксить? Просто в сервис передавать несколько реп?
...
// func (p *Postgres) AddUser(ctx context.Context, login string, password string) (int64, error) {
